...
# Y -> X
# Z -> Y
def c2e(front, right, back, left, top, bottom):
    posx = Image.fromarray(front, mode="RGB")
    negx = Image.fromarray(back, mode="RGB")
    posy = Image.fromarray(right, mode="RGB")
    negy = Image.fromarray(left, mode="RGB")
    posz = Image.fromarray(top, mode="RGB")
    negz = Image.fromarray(bottom, mode="RGB")
    ...
    for loopY in range(0,int(outputHeight)):# 0..height-1 inclusive
            for loopX in range(0,int(outputWidth)):
                ...
                output.append(
                    getColour(cart["x"],cart["y"],cart["index"],posx,negx,posy,negy,posz,negz)
                )
